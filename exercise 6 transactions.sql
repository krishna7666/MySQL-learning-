...
-- Q). Write a transaction to transfer $5000 from Alice's salary to Bob's salary. Rollback the transaction if Bob's salary exceeds $60,000 after the update.

start transaction;


-- Q). Create a transaction to update the salaries of multiple employees. Use a savepoint to rollback the update of one specific employee while committing others.
...
-- Q). Create a temporary table to calculate the average salary per department. Use the table to fetch departments with an average salary above $70,000.
CREATE TEMPORARY TABLE temp_avg_salary AS   SELECT department_id, avg(salary) as avg_salary from  employees group by department_id ;
select department_id ,avg_salary from temp_avg_salary where avg_salary > 70000;
...
DELIMITER $$
create event archived_employee_2
